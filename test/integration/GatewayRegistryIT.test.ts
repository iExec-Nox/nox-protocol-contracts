...
describe("[IT] GatewayRegistry", function () {
    it("Should register and remove gateway successfully", async function () {
        const { gatewayRegistry, wallet0, wallet1 } = await loadFixture();
        const gatewayAddress = wallet1.account.address;
        ...
        // Check initial state
        assert.strictEqual(true, await hasDefaultAdminRole(wallet0.account.address));
        assert.strictEqual(false, await hasGatewayRole(gatewayAddress));
        // Grant GATEWAY_ROLE to wallet1
        ...
        // Revoke GATEWAY_ROLE from wallet1
