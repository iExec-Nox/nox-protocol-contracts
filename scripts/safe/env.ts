...
const envSchema = z.object({
    ...
    RPC_URL: z.string().url("RPC_URL must be a valid URL").min(1, "RPC URL is required"),

    // Safe multisig contract address
    SAFE_ADDRESS: z.string().regex(addressRegex, "Invalid Safe address format").min(1, "Safe address is required"),
    ...
    TRANSACTION_DATA: z.string().regex(hexDataRegex, "Transaction data must be valid hex data").default("0x"),
